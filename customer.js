...
window.updateCustomerDatalist = function() { 
    ...
    if(dlist) { 
        let opts = ''; 
        window.customers.forEach(c => { opts += `<option value="${c.name}">${c.phone||''}</option>`; }); 
...
window.autoFillCustomerPhone = function(mode) { 
    ...
    const found = window.customers.find(c => String(c.name).toLowerCase() === nameInput.value.trim().toLowerCase()); 
    if(found && !phoneInput.value) phoneInput.value = found.phone ? found.phone : ''; 
};

window.setAutoRegisterCustomer = function(name, phone) { 
    if(!name) return; 
    const existing = window.customers.find(c => String(c.name).toLowerCase() === String(name).toLowerCase()); 
    if(!existing) window.customers.push({ id: 'C_' + Date.now(), name: name, phone: phone ? phone : '' }); 
    else if (!existing.phone && phone) existing.phone = phone; 
};

window.renderCustomers = function() {
    const search = document.getElementById('searchCustomer'); if(!search) return; 
    ...
    window.invoices.forEach(inv => { 
        ...
        if(inv.status === 'paid') custStats[inv.customer].paid += inv.totalAmount; 
        else if(inv.status === 'unpaid' || inv.status === 'preorder') { 
            custStats[inv.customer].paid += invPaid; 
            custStats[inv.customer].unpaid += (inv.totalAmount - invPaid); 
        }
    });
    const filtered = window.customers.filter(c => String(c.name).toLowerCase().includes(term) || (c.phone && String(c.phone).includes(term))); 
...
window.viewCustomerHistory = function(customerName) {
    ...
    const custInvoices = window.invoices.filter(inv => String(inv.customer).toLowerCase() === String(customerName).toLowerCase()); 
    let fHtml = ''; 
    custInvoices.forEach(inv => { 
        ...
        fHtml += `<tr><td style="font-size:var(--fs-12); color:var(--text-muted);">${inv.date}</td><td style="font-size:var(--fs-12);" title="${inv.items.map(i => i.name).join(', ')}">${itemsSummary}</td><td style="font-weight:bold; color:var(--success);">${window.fMoney(inv.totalAmount)}</td><td>${statusBadge}</td><td><button class="btn btn-outline" style="padding: 4px 8px; font-size: var(--fs-12);" onclick="window.viewInvoice('${inv.id}')">👁️ វិក្កយបត្រ</button></td></tr>`; 
    });
    document.getElementById('customerHistoryTableBody').innerHTML = fHtml || '<tr><td colspan="5" style="text-align:center;">អតិថិជននេះមិនទាន់មានប្រវត្តិទិញទេ</td></tr>'; 
...
window.saveCustomer = function() {
    ...
    if(id) { 
        const idx = window.customers.findIndex(c => c.id === id); 
        if(idx !== -1) { 
            const oldName = window.customers[idx].name; 
            if(oldName !== name) window.invoices.forEach(inv => { if(inv.customer === oldName) inv.customer = name; }); 
            window.customers[idx].name = name; 
            window.customers[idx].phone = phone; 
        } 
    } else { 
        if(window.customers.find(c => String(c.name).toLowerCase() === String(name).toLowerCase())) return window.ksMsg("ឈ្មោះអតិថិជននេះមានរួចហើយ!"); 
...
window.editCustomer = function(id) { 
    const c = window.customers.find(x => x.id === id); if(!c) return; 
...
window.deleteCustomer = function(id) { 
    window.ksMsg('តើអ្នកពិតជាចង់លុបអតិថិជននេះមែនទេ?', 'បញ្ជាក់ការលុប', true, () => { 
        let idx = window.customers.findIndex(c => c.id === id);
...
window.exportCustomers = function() { 
    if(!window.customers.length) return window.ksMsg('គ្មានទិន្នន័យអតិថិជនដើម្បី Export ទេ!'); 
        ...
        try { 
            ...
            if(Array.isArray(data)) { 
                ...
                data.forEach(newCust => { 
                    if(!window.customers.find(c => String(c.name).toLowerCase() === String(newCust.name).toLowerCase())) { 
